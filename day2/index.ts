...
function part1 () {
  ...
  let validCount = 0
  for (let i = 0; i < len; ++i) {
    let [minMax, alpha, pass] = realData[i].split(' ')
    alpha = alpha.substr(0, 1)
    const [min, max] = minMax.split('-').map(i => parseInt(i))
    const count = pass.slice().split('').filter(char => char === alpha).length
...
function part2 () {
  ...
  let validCount = 0
  for (let i = 0; i < len; ++i) {
    let [positions, alpha, pass] = realData[i].split(' ')
    alpha = alpha.substr(0, 1)
    const [pos1, pos2] = positions.split('-').map(i => parseInt(i))
    if ((pass[pos1 - 1] === alpha && pass[pos2 - 1] !== alpha) ||
      (pass[pos1 - 1] !== alpha && pass[pos2 - 1] === alpha)) {
      validCount++
    }
  }
